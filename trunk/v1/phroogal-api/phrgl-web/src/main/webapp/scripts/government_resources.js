...
angular.module('government-resources', ['common-login'])
		.config([ '$locationProvider', function($locationProvider) {
			$locationProvider.html5Mode(true);
		} ])
		.service('governmentResourcesService',['$http', '$location', function($http, $location) {
		return {
		...
		.controller('governmentResourcesCtrl',
				['$scope','governmentResourcesService',function($scope, governmentResourcesService) {
					var userObj = null;
					...
					$scope.initFunctionCall = function(){
						...
						if(tagsArray == null || tagsArray == undefined || tagsArray.length <=0){
							var allCategoryPromise = governmentResourcesService.getAllTags();
					...
					$scope.addNewRightQuestionButton=function(){
						...
						var questionObjPromise = governmentResourcesService.addNewQuestion($scope.rightNewQuestionTitle,content,rightAllCategories,isAnonymousFlag);
					...
					$scope.addUserTag = function(tagVal,user_id) {
						var userObjPromise = governmentResourcesService.addTagByUserId(tagVal,user_id);
					...
					$scope.showAllTrendingQuestions = function(){
						var allTrendingQuestionsPromise = governmentResourcesService.getTrendingQuestion();
