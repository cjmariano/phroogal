		...
		*/
		forgotPassword : function(_email) {
			...
			$http({ headers:header_encoding,method: 'POST', url: 'api/password-reset-request', data: data }).
			  success(function (data, status, headers, config) {
				  if (data.status == 'ERROR') {
				      //showErrorMessage("<strong>Oops!</strong><span> It seems like your email is not registered.</span>");
					  ...
					  //data-dismiss="modal" data-target="#forgotPasswordFormModel"					  
					  //showErrorMessage("<strong>Oops!</strong><span> Exception in sending mail.</span>");
					  ...
					  $("#forgotPasswordFormModel").hide();
					  //showErrorMessage("<span>Password link has been sent to your  email. Please check your email.</span>");
		...
		resetPassword : function(_password) {
			...
			$http({ headers:header_encoding,method: 'POST', url: 'password-reset-request-'+userId+'/password', data: Data  }).
			  success(function (data, status, headers, config) {				
				  if (data.status == 'SUCCESS') {
					  landingRedirectFlag=true;
					  	showErrorMessage("Change successful.");
					    //var ind=BASE_URL.indexOf("/api");
		    			//var url = BASE_URL.substring(0,ind + 1)+"signin?reset=true";					  
