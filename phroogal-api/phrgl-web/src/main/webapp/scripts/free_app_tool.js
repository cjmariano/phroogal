...
var trendingPageIndex=0;
	angular.module('free_app_tool', ['common-login']).
	config(['$locationProvider', function($locationProvider) {
	$locationProvider.html5Mode(true);
	}])
	.service('freeAppToolService',['$http', '$location', function($http, $location) {		
	return {
	...
	.controller('freeAppToolCtrl',
			[ '$scope', 'freeAppToolService', function($scope, freeAppToolService) {
		...
		$scope.initFunctionCall = function(){
			...
			if(tagsArray == null || tagsArray == undefined || tagsArray.length <=0){
				var allCategoryPromise = freeAppToolService.getAllTags();
		...
		$scope.addNewRightQuestionButton=function(){
			...
			var questionObjPromise = freeAppToolService.addNewQuestion($scope.rightNewQuestionTitle,content,rightAllCategories,isAnonymousFlag);
		...
		$scope.addUserTag = function(tagVal,user_id) {
			var userObjPromise = freeAppToolService.addTagByUserId(tagVal,user_id);
		...
		$scope.showAllTrendingQuestions = function(){
			var allTrendingQuestionsPromise = freeAppToolService.getTrendingQuestion();
